...
open() ->
    call_port({open}).
%%
%% add(Pathname, EventList) -> {ok, Wd} | {error, Reason}
%%
add(Pathname, EventList) when is_list(Pathname),
				  (is_atom(EventList) orelse is_list(EventList)) ->
    call_port({add, Pathname, EventList}).
...
%%
%% remove(Wd) -> ok| {error, Reason}
%%
remove(Wd) when is_integer(Wd) ->
    call_port({remove, Wd}).
%%
%% list() -> [Fds]
%%
