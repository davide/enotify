...
// This is the inotify file descriptor (as used by all functions)
ulong inotify_fd = -1;
...
int note_open(note_t *note, char *buf, int *count) {
  ...
  /* Prepare the output buffer that will hold {ok, opened} or {error, Reason} */
  if (ei_x_new_with_version(&result) ||
      ei_x_encode_tuple_header(&result, 2)) return(-1);
  ...
  newent->fd = inotify_fd;
  newent->next = note->ent;
  note->ent = newent;


  if (ei_x_encode_atom(&result, "ok") ||
      ei_x_encode_atom(&result, "opened")) return(-1);
...
int note_close(note_t *note, char *buf, int *count) {
  ...
  for(curpp = &(note->ent); *curpp != NULL; curpp = &(*curpp)->next) {
    if (inotify_fd == (*curpp)->fd) {
      inotify_fd = -1;
      close(inotify_fd);
      nextp = (*curpp)->next;
      free(*curpp);
      (*curpp) = nextp;
      /* Prepare the output buffer that will hold {ok, closed} or {error, Reason} */
      if (ei_x_new_with_version(&result) ||
	  ei_x_encode_tuple_header(&result, 2)) return(-1);
      if (ei_x_encode_atom(&result, "ok") ||
	  ei_x_encode_atom(&result, "closed")) return(-1);
...
int note_add(note_t *note, char *buf, int *index) {
  ...
  if (ei_decode_string(buf, index, pathname)) return(-1);
  ...
  if ((wd = inotify_add_watch(inotify_fd, pathname, mask)) < 0) {
...
int note_remove(note_t *note, char *buf, int *index) {
  ...
  if (ei_decode_ulong(buf, index, &wd)) return(-1);
